...
class RuleConfigTest {
  ...
  void shouldUseConfiguredOutOfHoursWindow() {
    TransactionContext context =
        context(
            new BigDecimal("100.00"), "MERCH_OK_002", "ZA", timestampAtSast(2024, 1, 1, 2, 15), 0);
  ...
  void shouldUseConfiguredVelocityRule() {
    TransactionContext context =
        context(
            new BigDecimal("100.00"), "MERCH_OK_003", "ZA", timestampAtSast(2024, 1, 1, 10, 0), 5);
    ...
    assertThat(ruleEngine.evaluate(context))
        ...
        .satisfies(
            result -> {
              ...
              assertThat(result.reason()).contains("6 transactions").contains("10 minutes");
            });
  }
