...
class SecurityConfigCorsTest {
    ...
    void cors_configuration_uses_explicit_origin_allowlist() {
        ...
        MockHttpServletRequest request =
                new MockHttpServletRequest(HttpMethod.GET.name(), "/api/v1/alerts");
        CorsConfiguration configuration = source.getCorsConfiguration(request);

        assertThat(configuration).isNotNull();
        assertThat(configuration.getAllowedOrigins())
                .containsExactly("https://fraud.capitec.example");
        ...
        assertThat(configuration.getExposedHeaders())
                .containsExactly(RequestCorrelationFilter.CORRELATION_HEADER);
    }

    @Test
    void cors_configuration_defaults_to_no_browser_origins() {
        CorsConfigurationSource source =
                securityConfig.corsConfigurationSource(new CorsProperties(List.of()));
